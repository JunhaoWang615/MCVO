...
#include "ThirdParty/ORBextractor/ORBextractor.h"
...
{
public:
	KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, vector<cv::Mat> &_image_list, vector<int> _point_num,
	...
	bool CompareSimilarity(std::vector<cv::Mat> &image1, std::vector<cv::Mat> &image2);
	void OpticalMatch(std::vector<cv::Point2f> &matched_2d_old,
						  std::vector<cv::Point2f> &matched_2d_old_norm,
                          std::vector<uchar> &status,
	...
	void PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
				   ...
				   int camera_index, 
				   KeyFrame* old_kf);
	void PnPRANSAC_op(	const vector<cv::Point2f> &matched_2d_old_norm,
						const std::vector<cv::Point3f> &matched_3d,
						std::vector<uchar> &status,
						Eigen::Vector3d &PnP_T_old, 
						Eigen::Matrix3d &PnP_R_old, 
	...
	void getcurPose(Eigen::Vector3d &_T_w_i, Eigen::Matrix3d &_R_w_i);
	...
	Eigen::Vector3d T_cur_in_old;		
	Eigen::Matrix3d R_cur_in_old;
	vector<cv::Mat> image_list; 
	vector<int> point_num; 
	vector<int> point_num_vector;
	vector<int> compute_point_num; 
	cv::Mat thumbnail;
