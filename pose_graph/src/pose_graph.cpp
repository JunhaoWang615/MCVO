...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
    ...
    pub_scale = n.advertise<sensor_msgs::PointCloud>("scales", 100);
    for (int i = 1; i < 10; i++)
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        ...
        cv::resize(keyframe->image_list[0], compressed_image, cv::Size(376, 240));
    ...
    {
        int min_index = -1;
        float min_index_score = 0;
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            if (min_index == -1 || ret[i].Id < min_index )
            {
                min_index = ret[i].Id;
                min_index_score = ret[i].Score;
            }
        }
        if (min_index_score < 0.02)
            min_index = -1;
        return min_index;
    }
    else
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            ceres::ScaledLoss* scaled_loss;
            scaled_loss = new ceres::ScaledLoss(loss_function, 5, ceres::TAKE_OWNERSHIP);
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                if((*it)->has_loop)
                {
                    ...
                    ceres::CostFunction* cost_function = FourDOFWeightError::Create( relative_t.x(), relative_t.y(), relative_t.z(),
                                                                               relative_yaw, euler_conncected.y(), euler_conncected.z());
                    problem.AddResidualBlock(cost_function, scaled_loss, euler_array[connected_index], 
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                Matrix3d tmp_r_tmp;
                Vector3d tmp_t_tmp;
                (*it)->getVioPose(tmp_t_tmp, tmp_r_tmp);
                if((*it)->index > cur_index - 100)
                    squared += (last_tmp_t - tmp_t).squaredNorm()/(last_tmp_t_tmp - tmp_t_tmp).squaredNorm();
                last_tmp_t_tmp = tmp_t_tmp;
                last_tmp_t = tmp_t;
            ...
            sensor_msgs::ChannelFloat32 scale_values;
            sensor_msgs::PointCloud path_scales;
            path_scales.header.stamp = ros::Time(cur_kf->time_stamp);
            double scale_aver = 0;
            if(i + 1 > 100)
                scale_aver = squared/100;
            else
                scale_aver = squared/(i + 1);

            scale_values.values.push_back(scale_aver);
            path_scales.channels.clear();
            path_scales.channels.push_back(scale_values);
            pub_scale.publish(path_scales);
            it++;
            for (; it != keyframelist.end(); it++)
            {
                Vector3d P;
...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        if (DEBUG_IMAGE)
        {
            ...
            imwrite(image_path.c_str(), (*it)->image_list[0]);
        }
