...
void readParameters(ros::NodeHandle &n)
{
    ...
    depth_support_init = fsSettings["depth_support_init"];
    ...
    else
    {
        ...
        // LOG(INFO) << "T_i_c Extrinsic_R : \n"
        //           << RIC[0];
        // LOG(INFO) << "T_i_c Extrinsic_T : \n"
        //           << TIC[0].transpose();
    }

    // cv::Mat cv_R_l, cv_T_l;
    // fsSettings["extrinsicRotation_imu_laser"] >> cv_R_l;
    // fsSettings["extrinsicTranslation_imu_laser"] >> cv_T_l;
    // cv::cv2eigen(cv_R_l, imu_laser_R);
    // cv::cv2eigen(cv_T_l, imu_laser_T);

    // Timu_lidar = Transformd(imu_laser_R, imu_laser_T);
    // LOG(INFO) << "\n T_imu_lidar: \n"
    //           << Timu_lidar.matrix();

    // Transformd T_imu_cam(RIC[0], TIC[0]);
    // Tcam_lidar = T_imu_cam.inverse() * Timu_lidar;
    // LOG(INFO) << "\n T_camera_lidar: \n"
    //           << Tcam_lidar.matrix();
