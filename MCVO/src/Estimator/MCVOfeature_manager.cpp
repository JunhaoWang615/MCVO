...
FeatureManager::FeatureManager(Matrix3d _Rs[], MCVOfrontend *frontend)
                                             ...
                                             double td)
{
    ...
    for (int c = 0; c < NUM_OF_CAM; c++)
    {
        ...
        for (auto &id_pts : image.results[c]->features)
        {
            ...
            MCVO::FeatureID feature_id = id_pts.first;
            // printf("----\n File: \"%s\" \n line: %d \n function <%s>\n Content: %d : %d \n  =====",
            //        __FILE__, __LINE__, __func__,
            //        c, feature_id);
...
void FeatureManager::removeOneFrameObservation(MCVO::TimeFrameId marg_frame_tid)
{
...
void FeatureManager::removeOneFrameObservationAndShiftDepth(MCVO::TimeFrameId marg_frame_tid,
                                                            Vector3d Ps[],
