...
void pubCameraPose(const MCVOEstimator &estimator, const std_msgs::Header &header)
{
  ...
  if (estimator.solver_flag == MCVOEstimator::SolverFlag::NON_LINEAR)
  {
    ...
    {
      if (sensor->type == MCVO::sensor_type::MONOCULAR ||
          sensor->type == MCVO::sensor_type::RGBD ||
          sensor->type == MCVO::sensor_type::THERMAL)
      {
...
void pubKeyframe(const MCVOEstimator &estimator)
{
  ...
  if (estimator.solver_flag == MCVOEstimator::SolverFlag::NON_LINEAR &&
      estimator.marginalization_flag ==
          MCVOEstimator::MarginalizationFlag::MARGIN_OLD)
  {
    int i = WINDOW_SIZE - 2;
