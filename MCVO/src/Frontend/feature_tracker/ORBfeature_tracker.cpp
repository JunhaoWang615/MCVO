#include "ORBfeature_tracker.h"
#include <fstream>

using namespace MCVO;

bool ORBFeatureTracker::inBorder(const cv::Point2f &pt)
{
...
void ORBFeatureTracker::reduceVector(vector<cv::KeyPoint> &v, vector<uchar> &status)
{
...
void ORBFeatureTracker::reduceMat1(cv::Mat &mat, vector<uchar> &status)
{
    ...
    int first_invalid_idx = mat.rows;
    ...
    cv::Mat matOut = mat(cv::Range(first_valid_idx, first_invalid_idx), cv::Range(0, mat.cols));
    cout << "matOut size:" << matOut.size() << endl;
    for (int i = first_invalid_idx; i < int(mat.rows); i++)
    {
        if (status[i])
        {
            cv::Mat row;
            row = mat(cv::Range(i, i + 1), cv::Range(0, mat.cols));
            cv::vconcat(matOut, row, matOut);
        }
    }
    mat = matOut;
    cout << "mat size:" << mat.size() << endl;
...
void ORBFeatureTracker::reduceMat(cv::Mat &mat, vector<uchar> &status)
{
    int j = 0;
    if (mat.rows == 0)
        return;
...
ORBFeatureTracker::ORBFeatureTracker()
{
    ...
    ORBextractor_ = std::make_shared<ORB_SLAM2::ORBextractor>(1000, 1.2, 8, 20, 7);
#else
    ORBdetector = cv::ORB::create(500, 1.2f, 8, 31, 0, 2, cv::ORB::HARRIS_SCORE, 31, 20);
#endif
    matcher = cv::DescriptorMatcher::create(cv::DescriptorMatcher::FLANNBASED);
    max_dist = 30;
}

void ORBFeatureTracker::setMask()
{
    if (cam->FISHEYE)
...
// 这里有一些疑惑
            ...
            cv::Mat tmp_desc;
            ORBextractor_ = std::make_shared<ORB_SLAM2::ORBextractor>(1000, 1.2, 8, 20, 7);
            LOG(INFO) << "Extract points using ORB extractor in ORB_SLAM2";
            ORBextractor_->setNfeatures(1000);
            (*ORBextractor_)(forw_img, fisheye_mask, tmp_pts, tmp_desc);
            LOG(INFO) << "Finish extraction";
#else
            ORBdetector->setMaxFeatures(500);
            vector<cv::KeyPoint> tmp_pts;
            cv::Mat tmp_desc;
            ...
            {
                // cout<<knn_matches[i][0].trainIdx<<endl;
                // cout<<"match 0:"<<knn_matches[i][0].distance<<endl;
                // cout<<"match 1:"<<knn_matches[i][1].distance<<endl;
                ...
                int idx = knn_matches[i][0].trainIdx;
                forw_pts.push_back(tmp_pts[idx]); 
                // cv::Mat row = tmp_desc(cv::Range(idx, idx + 1), cv::Range(0, tmp_desc.cols));
        ...
        else
        {
            ...
            // LOG(INFO)<<"forw_desc size: "<<forw_desc.size();
        }
...
#if USE_ORB_SLAM2_DETECTOR
            // ORBextractor_->setNfeatures(n_max_cnt);
            ORBextractor_ = std::make_shared<ORB_SLAM2::ORBextractor>(n_max_cnt, 1.2, 8, 20, 7);
            (*ORBextractor_)(forw_img, fisheye_mask, n_pts, n_desc);
            // cout << "after n_max_cnt n_pts "<< "should" << n_max_cnt << "  but  " << n_pts.size() <<endl;


            //这里也有，因为只是选取最好的点。可能大部分点与上一步ORB_detec有重复的点
            n_desc.convertTo(n_desc, 5);
#else
            ORBdetector->setMaxFeatures(n_max_cnt);
            ORBdetector->detectAndCompute(forw_img, fisheye_mask, n_pts, n_desc);
            n_desc.convertTo(n_desc, 5);
#endif
            // ORBdetector->detect(forw_img, n_pts);
            // ORBdetector->compute(forw_img, n_pts, n_desc);
...
void ORBFeatureTracker::rejectWithF()
{
    if (forw_pts.size() >= 8)
    {
        ...
        vector<uchar> status;
        cv::findFundamentalMat(un_cur_pts, un_forw_pts, cv::FM_LMEDS, cam->F_THRESHOLD, 0.99, status);
        ...
        // cout << "reduce cur_desc" << endl;
        reduceMat(cur_desc, status);
        // LOG(INFO) << "Pts num After ransac: " << cur_pts.size();
    }
}

bool ORBFeatureTracker::updateID(unsigned int i)
{
    if (i < ids.size())
    {
        if (ids[i] == -1)
...
void ORBFeatureTracker::readIntrinsicParameter(const string &calib_file)
{
    ROS_INFO("reading paramerter of camera %s", calib_file.c_str());
    m_camera = CameraFactory::instance()->generateCameraFromYamlFile(calib_file);
}

void ORBFeatureTracker::showUndistortion(const string &name)
{
...
void ORBFeatureTracker::undistortedPoints()
{
    cur_un_pts.clear();
...
void ORBFeatureTracker::getPt(int idx, int &id, geometry_msgs::Point32 &p, cv::Point2f &p_uv, cv::Point2f &v)
{
    id = ids[idx];
...
void ORBFeatureTracker::getCurPt(int idx, cv::Point2f &cur_pt)
{
